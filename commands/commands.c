/* Generated from ./commands/commands.c0 at 06/03/2022 */ 
	...
	do {
		...
		if (token == NULL || streq(token, ">")) {
			...
			if (streq(command, "randomstate")) {
				/* Imported from ./commands/randomstate_cmd.ct at 06/03/2022 */ 
	...
	*selection = (state*) random_state(opt_shape);
	...
	selection_type = "state_set";
			...
			// TODO: exploit symmetries and pattern components for more compact storage/representation?
				...
				/* Imported from ./commands/enumerate_cmd.ct at 06/03/2022 */ 
...
selection_type = "state_set";
				...
				/* Imported from ./commands/write_cmd.ct at 06/03/2022 */ 
...
complete = 1;
				...
				/* Imported from ./commands/print_cmd.ct at 06/03/2022 */ 
if (streq(selection_type, "simulation")) {
	...
	for (int j=0; j<opt_num; j++) {
		sim_summary(selection[j]);
				...
				/* Imported from ./commands/render_cmd.ct at 06/03/2022 */ 
...
if (streq(selection_type, "state")) {
	write_image(*((state*) selection), opt_color);
				...
				/* Imported from ./commands/simulate_cmd.ct at 06/03/2022 */ 
